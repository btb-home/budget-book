...
class SafeRotatingFileHandler(TimedRotatingFileHandler):
    ...
    def __init__(
        ...
        atTime: (
            datetime | None
        ) = None,  # 특정 시간에 롤오버를 수행하도록 지정 (기본값: None)
    ):
    ...
    def doRollover(self):
        ...
        dst_now = time.localtime(current_time)[
            -1
        ]  # 현재 시간의 DST 상태 (서머타임 여부)
        ...
        if not os.path.exists(self.baseFilename) and not os.path.lexists(
            self.baseFilename
        ):
