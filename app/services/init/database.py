...
def _insert_user_account(db_session: Session, data: UserAccountReq):
    """
    UserAccount 데이터를 데이터베이스에 삽입합니다.
    이미 데이터가 있는 경우 업데이트를 수행합니다.
    """
    try:
        insert(db_session, UserAccount, UserAccountReq)(data)
    except DuplicateDataException as e:
        table_name = UserAccount.__tablename__
        LOGGER.warning(f"[   DB] {e.detail} - 테이블: {table_name} 데이터: {data.id}")
